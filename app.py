...
def infer_weights():
    """Infer weights using LP optimization to maximize scores of selected parcels"""
    data = request.get_json()
    selection_polygon = data.get('selection')
    max_parcels = data.get('max_parcels', 500)  # Budget constraint
    ...
    # Get parcels within selection with their scores
    cur.execute("""
    ...
    prob = LpProblem("Maximize_Selected_Parcel_Scores", LpMaximize)
    
    # Create weight variables
    weight_vars = LpVariable.dicts("weight", WEIGHT_VARS, lowBound=0)
    
    # Objective: Maximize sum of scores for selected parcels
    prob += lpSum(
        weight_vars[var] * float(p.get(var, 0) or 0)
        for p in selected_parcels
        for var in WEIGHT_VARS
    )
    
    # Constraint: Weights must sum to 1
    prob += lpSum(weight_vars.values()) == 1
    
    # Solve the problem
    prob.solve()
    
    # Get the optimal weights
    best_weights = {var: value(weight_vars[var]) for var in WEIGHT_VARS}
    
    # Convert to percentages
    weights_pct = {k: round(v * 100, 1) for k, v in best_weights.items()}
    
    # Calculate how many selected parcels would be in top N using the database
    score_formula = " + ".join([f"COALESCE({var}, 0) * %s" for var in WEIGHT_VARS])
    params = [best_weights.get(var, 0) for var in WEIGHT_VARS]
    ...
    cur.execute(f"""
        ...
        SELECT COUNT(*) as num_selected_in_top
        FROM ranked_parcels r
        JOIN (
        ...
        WHERE r.rank <= %s
    """, params + [json.dumps(selection_polygon), max_parcels])
    
    num_selected_in_top = cur.fetchone()['num_selected_in_top']
    ...
    return jsonify({
        ...
        "message": f"Found weights that select {num_selected_in_top} of your {len(selected_parcels)} parcels within the budget of {max_parcels} parcels"
    })
