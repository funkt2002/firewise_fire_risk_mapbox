        ...
        // ========== PHASE 2A: COMPONENT LAZY LOADER ==========
        
        /**
         * ComponentLazyLoader - Handles lazy loading of disabled weight sliders
         * Part of Phase 2A optimization: Reduces initial DOM by 115+ elements
         */
        class ComponentLazyLoader {
            constructor() {
                this.loadedSliders = new Set();
                this.loadingSliders = new Set();
                
                // Weight slider configurations for disabled sliders
                this.sliderConfigs = {
                    'neigh1d_s': {
            ...
            /**
             * Load a weight slider from placeholder to full component
             * @param {string} sliderId - The slider ID to load
             */
            async loadWeightSlider(sliderId) {
                if (this.loadedSliders.has(sliderId) || this.loadingSliders.has(sliderId)) {
                    console.log(`Slider ${sliderId} already loaded or loading`);
                    return;
                }
                
                if (!this.sliderConfigs[sliderId]) {
                    console.error(`Unknown slider ID: ${sliderId}`);
                    return;
                }
                
                this.loadingSliders.add(sliderId);
                
                try {
                    const placeholderElement = document.querySelector(`[data-variable="${sliderId}"][data-lazy-load="true"]`);
                    if (!placeholderElement) {
                        console.error(`Placeholder not found for slider: ${sliderId}`);
                        return;
                    }
                    
                    // Add loading state
                    placeholderElement.classList.add('loading');
                    
                    // Create the full slider HTML
                    const config = this.sliderConfigs[sliderId];
                    const sliderHTML = this.generateSliderHTML(sliderId, config);
                    
                    // Create temporary container
                    const tempDiv = document.createElement('div');
                    tempDiv.innerHTML = sliderHTML;
                    const newSliderElement = tempDiv.firstElementChild;
                    
                    // Smooth transition: fade out placeholder
                    placeholderElement.style.opacity = '0.5';
                    
                    // Wait for fade transition
                    await new Promise(resolve => setTimeout(resolve, 150));
                    
                    // Replace placeholder with full slider
                    placeholderElement.parentNode.replaceChild(newSliderElement, placeholderElement);
                    
                    // Initialize the new slider
                    this.initializeSlider(sliderId, newSliderElement);
                    
                    // Fade in the new slider
                    newSliderElement.style.opacity = '0';
                    await new Promise(resolve => setTimeout(resolve, 50));
                    newSliderElement.style.transition = 'opacity 0.3s ease';
                    newSliderElement.style.opacity = '1';
                    
                    this.loadedSliders.add(sliderId);
                    
                    console.log(`✅ Lazy loaded slider: ${sliderId}`);
                    
                } catch (error) {
                    console.error(`Error loading slider ${sliderId}:`, error);
                } finally {
                    this.loadingSliders.delete(sliderId);
                }
            }
            
            /**
             * Generate HTML for a full weight slider
             * @param {string} sliderId - The slider ID
             * @param {Object} config - The slider configuration
             * @returns {string} HTML string for the slider
             */
            generateSliderHTML(sliderId, config) {
                return `
                <div class="variable-container" data-variable="${sliderId}">
                    <div class="variable-header">
                        <input type="checkbox" class="variable-enable-checkbox" id="enable-${sliderId}" checked>
                        <span class="variable-name">${config.name}</span>
                    </div>
                    <div class="variable-controls expanded">
                        ${config.subtitle ? `<div style="font-size: 10px; color: #888; margin-bottom: 3px;">${config.subtitle}</div>` : ''}
                        <div class="slider-container">
                            <div class="slider-label">
                                <span class="slider-value" id="${sliderId}-value">${config.value}%</span>
                            </div>
                            <input type="range" class="slider weight-slider" id="${sliderId}" min="0" max="100" value="${config.value}">
                            <div class="dist-buttons">
                                <button class="dist-button" onclick="window.enhancedPlottingManager.showDistribution('${config.raw_key}')">Raw</button>
                                <button class="dist-button" onclick="window.enhancedPlottingManager.showDistribution('${sliderId}')">Score</button>
                                <button class="dist-button" onclick="window.enhancedPlottingManager.showVariableCorrelation('${config.correlation_key}')">Correlation</button>
                            </div>
                        </div>
                    </div>
                </div>`;
            ...
            /**
             * Initialize event listeners for a newly loaded slider
             * @param {string} sliderId - The slider ID
             * @param {Element} sliderElement - The slider DOM element
             */
            initializeSlider(sliderId, sliderElement) {
                // Find the slider input and value display
                const sliderInput = sliderElement.querySelector(`#${sliderId}`);
                const valueDisplay = sliderElement.querySelector(`#${sliderId}-value`);
                const checkbox = sliderElement.querySelector(`#enable-${sliderId}`);
                ...
                if (sliderInput && valueDisplay) {
                    ...
                    sliderInput.addEventListener('input', (e) => {
                        ...
                        // Trigger weight update if needed
                ...
                if (checkbox) {
                    ...
                    checkbox.addEventListener('change', (e) => {
                        const controls = sliderElement.querySelector('.variable-controls');
                        if (controls) {
                            if (e.target.checked) {
                                controls.classList.add('expanded');
                                controls.classList.remove('collapsed');
                ...
                console.log(`Initialized event listeners for slider: ${sliderId}`);
            }
            
            /**
             * Get count of loaded sliders for metrics
             */
            getLoadedSlidersCount() {
                return this.loadedSliders.size;
            }
            
            /**
             * Get list of loaded slider IDs
             */
            getLoadedSliderIds() {
                return Array.from(this.loadedSliders);
            }
        }
        
        // Initialize the ComponentLazyLoader globally
        window.componentLazyLoader = new ComponentLazyLoader();
        
        // Add to fireRiskApp services
        if (window.fireRiskApp && window.fireRiskApp.services) {
            window.fireRiskApp.services.componentLazyLoader = window.componentLazyLoader;
        }
        
        console.log('✅ Phase 2A: ComponentLazyLoader initialized with 5 disabled slider configurations');
