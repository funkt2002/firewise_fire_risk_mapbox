...
def solve_weight_optimization_test(parcel_data, include_vars):
    """Test version of the LP solver"""
    
    # Process variable names
    include_vars_base = [var[:-2] if var.endswith(('_s', '_q')) else var for var in include_vars]
    
    print(f"    Solving LP with {len(parcel_data):,} parcels, {len(include_vars_base)} variables")
...
def run_test_scenario(scenario_name, sizes, data_types, variable_exclusions, quantile_convert=False):
    """Run a complete test scenario"""
    quantile_suffix = " (QUANTILE)" if quantile_convert else ""
    print(f"\n{'='*60}")
    print(f"RUNNING SCENARIO: {scenario_name}{quantile_suffix}")
    print(f"{'='*60}")
        ...
        for data_type in data_types:
            for exclude_vars in variable_exclusions:
                ...
                weights, total_score, solve_time, success, coefficients = solve_weight_optimization_test(
                    parcel_data, active_vars
                )
                
                # Store results
                result = {
                    'dataset': f"{data_type}" + (f"_excl{len(exclude_vars)}" if exclude_vars else "") + ("_q" if quantile_convert else ""),
                    'size': size,
                ...
                if success:
                    ...
                    # Show more balanced results if quantile
                    if quantile_convert:
                        sorted_weights = sorted(weights.items(), key=lambda x: x[1], reverse=True)
                        significant_vars = [(var, weight) for var, weight in sorted_weights if weight > 0.05]
                        if len(significant_vars) > 1:
                            print(f"    Significant variables (>5%): {[(var, f'{weight:.2f}') for var, weight in significant_vars[:3]]}")
                else:
...
def main():
    """Main test runner"""
    print("FIRE RISK LP OPTIMIZATION TEST SUITE")
    print("=" * 50)
    
    # Test configuration - more scenarios for comprehensive testing
    sizes = [100, 1000, 10000]  # Added smaller size for quick testing
    
    # Test data types
    all_data_types = ['random', 'high_skew', 'normal_skew', 'correlated', 'edge_all_high', 'edge_all_low', 'edge_sparse']
    basic_exclusions = [None]  # No exclusions
    
    print("\n" + "="*80)
    print("TESTING RAW vs QUANTILE APPROACH COMPARISON")
    print("="*80)
    
    # Test RAW scores (current approach)
    print(f"\n🔹 TESTING WITH RAW SCORES (Current Approach)")
    raw_results = run_test_scenario(
        "RAW SCORES - ALL DISTRIBUTIONS", 
        sizes, 
        all_data_types, 
        basic_exclusions,
    ...
    # Test QUANTILE scores (proposed approach)
    print(f"\n🔹 TESTING WITH QUANTILE SCORES (Proposed Approach)")
    quantile_results = run_test_scenario(
        "QUANTILE SCORES - ALL DISTRIBUTIONS", 
        sizes, 
        all_data_types, 
        basic_exclusions,
        quantile_convert=True
    )
    
    # Combine all results
    all_results = raw_results + quantile_results
    
    # Print comparison summary
    print(f"\n{'='*100}")
    print("RAW vs QUANTILE COMPARISON SUMMARY")
    print(f"{'='*100}")
    
    print(f"{'Dataset':<15} {'Size':<8} {'RAW Top Var':<15} {'RAW Weight':<12} {'QUANTILE Top Var':<18} {'QUANTILE Weight':<15}")
    print("-" * 100)
    ...
    for result in all_results:
        ...
        if result['quantile']:
            quantile_dict[key] = result
        else:
            raw_dict[key] = result
    
    for key in sorted(raw_dict.keys()):
        if key in quantile_dict:
            raw_result = raw_dict[key]
            quantile_result = quantile_dict[key]
            
            if raw_result['success'] and quantile_result['success']:
                # Get top variable for each
                raw_top = max(raw_result['weights'].items(), key=lambda x: x[1])
                quantile_top = max(quantile_result['weights'].items(), key=lambda x: x[1])
                
                dataset = f"{key[0]}"
                size = f"{key[1]:,}"
                raw_var = f"{raw_top[0]}"
                raw_weight = f"{raw_top[1]:.3f}"
                quantile_var = f"{quantile_top[0]}"
                quantile_weight = f"{quantile_top[1]:.3f}"
                
                print(f"{dataset:<15} {size:<8} {raw_var:<15} {raw_weight:<12} {quantile_var:<18} {quantile_weight:<15}")
    
    # Count how many scenarios show more balanced weights with quantile
    print(f"\n{'='*100}")
    ...
    for key in raw_dict:
        if key in quantile_dict:
            ...
            if raw_result['success'] and quantile_result['success']:
                ...
                raw_significant = sum(1 for w in raw_result['weights'].values() if w > 0.1)
                quantile_significant = sum(1 for w in quantile_result['weights'].values() if w > 0.1)
                
                if raw_significant > 1:
                    raw_balanced += 1
                if quantile_significant > 1:
                    quantile_balanced += 1
    
    print(f"Scenarios with balanced weights (>1 variable with >10% weight):")
    print(f"  Raw scores:     {raw_balanced}/{total_comparisons} ({100*raw_balanced/total_comparisons:.1f}%)")
    print(f"  Quantile scores: {quantile_balanced}/{total_comparisons} ({100*quantile_balanced/total_comparisons:.1f}%)")
    
    # Show detailed analysis for most interesting quantile cases
    print(f"\n{'='*100}")
    print("DETAILED QUANTILE WEIGHT ANALYSES (Most Balanced Cases)")
    print(f"{'='*100}")
    
    # Sort quantile results by how balanced they are
    quantile_only = [r for r in all_results if r['quantile'] and r['success']]
    quantile_only.sort(key=lambda x: sum(1 for w in x['weights'].values() if w > 0.05), reverse=True)
    
    # Show top 10 most balanced
    for result in quantile_only[:10]:
        if result['size'] >= 1000:  # Only show meaningful sizes
            print_weight_analysis(result)
    
    print(f"\n✓ Test suite completed. Processed {len(all_results)} scenarios.")
    print(f"✓ Key finding: {'Quantile approach shows more balanced weights!' if quantile_balanced > raw_balanced else 'Raw and quantile approaches show similar balance patterns.'}")
